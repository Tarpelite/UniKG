...
from pytorch_pretrained_bert.modeling import BertForPreTrainingLossMask, BertForUniKG
...
def main():
    ...
    if args.do_train:
        ...
        bi_uni_pipeline = [Preprocess4Kp20k(args.max_pred, args.mask_prob, list(tokenizer.vocab.keys(
        ...
        train_dataset = Kp20kDataset(
            fn_src, args.train_batch_size, data_tokenizer, args.max_seq_length, file_oracle=file_oracle, bi_uni_pipeline=bi_uni_pipeline)
    ...
    if (recover_step is None) and (args.model_recover_path is None):
        ...
        model = BertForUniKG.from_pretrained(
    ...
    if args.do_train:
        ...
        for i_epoch in trange(start_epoch, int(args.num_train_epochs)+1, desc="Epoch", disable=args.local_rank not in (-1, 0)):
            ...
            for step, batch in enumerate(iter_bar):
                ...
                try:
                    print("pos_seq", Counter(pos_seq.view(-1).detach().cpu().numpy()))
                    print("pre_seq", Counter(pre_seq.view(-1).detach().cpu().numpy()))
